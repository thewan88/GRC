...
export default function Step6Controls({ control_ids, onChange }: Props) {
  ...
  const byTheme = useMemo(() => {
    ...
    for (const c of filtered) {
      if (c.theme in map) map[c.theme].push(c);
    }
    return map;
  }, [filtered]);

  function toggle(id: string) {
    const next = control_ids.includes(id)
      ? control_ids.filter((c) => c !== id)
      : [...control_ids, id];
    onChange(next);
  }
    ...
    <div className="space-y-4">
      <div>
        <p className="mb-3 text-sm text-gray-600">
          Link ISO 27001:2022 Annex A controls that mitigate this risk. Selected: <strong>{control_ids.length}</strong>
        </p>
        ...
        <input
          ...
          placeholder="Search controls by reference, name or keyword..."
      ...
      <div className="max-h-80 overflow-y-auto rounded-lg border border-gray-200">
        {THEME_ORDER.map((theme) => {
          ...
          const themeSelected = themeControls.filter((c) => control_ids.includes(c.id)).length;
            ...
            <div key={theme} className="border-b border-gray-200 last:border-b-0">
              ...
              >
                ...
                <span className="flex items-center gap-2">
                  {themeSelected > 0 && (
                    <span className="rounded-full bg-blue-100 px-1.5 py-0.5 text-xs font-medium text-blue-700">{themeSelected}</span>
                  )}
                  ...
                  {themeControls.map((c) => {
                    ...
                    return (
                      <label
                        key={c.id}
                        className={cn(
                          'flex cursor-pointer items-start gap-3 px-4 py-2.5 hover:bg-gray-50',
                          selected && 'bg-blue-50',
                        )}
                      >
                        <input
